<div class="space-y-6">
        <div class="max-w-xl">
            <form wire:submit.prevent="assign" class="flex gap-3">
                <flux:select wire:model.defer="teacher_id" class="w-64" label="Maestro">
                    <option value="">Seleccione</option>
                    @foreach($teachers as $t)
                        <option value="{{ $t->id }}">{{ $t->name }}</option>
                    @endforeach
                </flux:select>
                <flux:select wire:model.defer="subject_id" class="w-64" label="Materia">
                    <option value="">Seleccione</option>
                    @foreach($subjects as $s)
                        <option value="{{ $s->id }}">{{ $s->name }}</option>
                    @endforeach
                </flux:select>
                <flux:button type="submit">Asignar</flux:button>
            </form>
        </div>

        <div class="overflow-x-auto">
            <table class="min-w-full text-sm border border-neutral-200 dark:border-neutral-700 rounded-xl overflow-hidden">
                <thead class="bg-neutral-50 dark:bg-neutral-800">
                    <tr>
                        <th class="px-4 py-2 text-left">Maestro</th>
                        <th class="px-4 py-2 text-left">Materia</th>
                        <th class="px-4 py-2 text-left w-32">Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($rows as $r)
                        <tr class="border-t border-neutral-200 dark:border-neutral-700">
                            <td class="px-4 py-2">{{ $r->teacher->name }}</td>
                            <td class="px-4 py-2">{{ $r->subject->name }}</td>
                            <td class="px-4 py-2">
                                <flux:button size="xs" color="red" wire:click="delete({{ $r->id }})">Eliminar</flux:button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
